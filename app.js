...
const openCheckOutButton =
  document.getElementById("openCheckOutButton");
...
const checkOutCancelButton =
  document.getElementById("checkOutCancelButton");

const checkOutPinInput =
  document.getElementById("checkOutPinInput");

const checkOutStatus =
  document.getElementById("checkOutStatus");

const confirmCheckOutButton =
  document.getElementById("confirmCheckOutButton");

const backToShiftFromCheckOutButton =
  document.getElementById("backToShiftFromCheckOutButton");
...
async function checkOutBarberByPin(pin) {
  if (!currentBranch) {
    throw new Error(
      "ยังไม่พบข้อมูลสาขา"
    );
  ...
  if (!isActiveHere) {
    throw new Error(
      "ช่างคนนี้ไม่ได้เข้างานอยู่ในสาขานี้"
    );
  }
...
function closeCheckOutModal() {
  ...
  if (
    ...
    closeStoreModal.classList.contains("hidden")
  ) {
...
if (openCheckOutButton) {
  openCheckOutButton.addEventListener(
    "click",
    openCheckOutModal
  );
}
...
if (checkOutCancelButton) {
  checkOutCancelButton.addEventListener(
    "click",
    closeCheckOutModal
  );
}
...
if (backToShiftFromCheckOutButton) {
  backToShiftFromCheckOutButton.addEventListener(
    "click",
    () => {
      closeCheckOutModal();
      openShiftModal();
...
if (checkOutPinInput) {
  checkOutPinInput.addEventListener(
    "input",
    () => {
      checkOutPinInput.value =
        checkOutPinInput.value
...
if (
  ...
  confirmCheckOutButton.addEventListener(
    ...
    async () => {
      ...
      confirmCheckOutButton.textContent =
        "กำลังออกงาน...";

      try {
        const barber =
          await checkOutBarberByPin(pin);

        closeCheckOutModal();

        showNotice(
          `${barber.name} ออกงานเรียบร้อย`,
          "ออกงานเรียบร้อย"
        );
